...
import (
	...
	"github.com/exohood/exohood-smart-chain/common"
	"github.com/exohood/exohood-smart-chain/crypto/signify"
	"github.com/exohood/exohood-smart-chain/internal/build"
	"github.com/exohood/exohood-smart-chain/params"
)
...
var (
	...
	debExecutables = []debExecutable{
		...
		{
			BinaryName:  "exo-sc",
			Description: "exohood CLI client.",
		},
		{
			BinaryName:  "puppeth",
		...
		{
			BinaryName:  "clef",
			Description: "exohood account management tool.",
		},
	}
...
func doArchive(cmdline []string) {
	var (
		...
		upload  = flag.String("upload", "", `Destination to upload the archives (usually "exo-sc-store/builds")`)
		ext     string
	)
	flag.CommandLine.Parse(cmdline)
	switch *atype {
	...
	var (
		...
		geth     = "exo-sc-" + basegeth + ext
		alltools = "exo-sc-alltools-" + basegeth + ext
	)
...
func archiveUpload(archive string, blobstore string, signer string, signifyVar string) error {
	...
	if signifyVar != "" {
		...
		untrustedComment := "verify with exo-sc-release.pub"
...
func doDocker(cmdline []string) {
	var (
		...
		upload   = flag.String("upload", "", `Where to upload the docker image (usually "exohood/client-go")`)
	)
	flag.CommandLine.Parse(cmdline)
...
func doDebianSource(cmdline []string) {
	var (
		...
		upload   = flag.String("upload", "", `Where to upload the source package (usually "exohood/exohood")`)
		sshUser  = flag.String("sftp-user", "", `Username for SFTP upload (usually "exo-sc-ci")`)
...
func makeWorkdir(wdflag string) string {
	...
	if wdflag != "" {
		...
		wdflag, err = os.MkdirTemp("", "exo-sc-build-")
	}
	if err != nil {
		log.Fatal(err)
	}
...
func doWindowsInstaller(cmdline []string) {
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, file := range allToolsArchiveFiles {
		...
		if filepath.Base(file) == "exo-sc.exe" {
			gethTool = file
		} else {
	...
	build.Render("build/nsis.exo-sc.nsi", filepath.Join(*workdir, "exo-sc.nsi"), 0644, nil)
	...
	installer, _ := filepath.Abs("exo-sc-" + archiveBasename(*arch, params.ArchiveVersion(env.Commit)) + ".exe")
	build.MustRunCommand("makensis.exe",
		...
		"/DARCH="+*arch,
		filepath.Join(*workdir, "exo-sc.nsi"),
	)
...
func doAndroidArchive(cmdline []string) {
	var (
		...
		upload  = flag.String("upload", "", `Destination to upload the archive (usually "exo-sc-store/builds")`)
	)
	flag.CommandLine.Parse(cmdline)
	env := build.Env()
	tc := new(build.GoToolchain)
	...
	build.MustRun(gomobileTool("bind", "-ldflags", "-s -w", "--target", "android", "--javapkg", "org.ethereum", "-v", "github.com/exohood/exohood-smart-chain/mobile"))

	if *local {
		// If we're building locally, copy bundle to build dir and skip Maven
		os.Rename("exo-sc.aar", filepath.Join(GOBIN, "exo-sc.aar"))
		os.Rename("exo-sc-sources.jar", filepath.Join(GOBIN, "exo-sc-sources.jar"))
		return
	}
	...
	archive := "exo-sc-" + archiveBasename("android", params.ArchiveVersion(env.Commit)) + ".aar"
	os.Rename("exo-sc.aar", archive)
...
func doXCodeFramework(cmdline []string) {
	...
	bind := gomobileTool("bind", "-ldflags", "-s -w", "--target", "ios", "-v", "github.com/exohood/exohood-smart-chain/mobile")
	...
	// Create the archive.
	maybeSkipArchive(env)
	archive := "exo-sc-" + archiveBasename("ios", params.ArchiveVersion(env.Commit))
	...
	if *deploy != "" {
		...
		build.Render("build/pod.podspec", "exo-SC.podspec", 0755, meta)
		build.MustRunCommand("pod", *deploy, "push", "exo-SC.podspec", "--allow-warnings")
	}
}
...
func doPurge(cmdline []string) {
	var (
		store = flag.String("store", "", `Destination from where to purge archives (usually "exo-sc-store/builds")`)
